""" Unit test module for env_util. """
import unittest
from src.api.util.env import environment_variable_proxy
from unittest.mock import patch


class EnvironmentUtilTestSuite(unittest.TestCase):
  """ Unit test suite for EnvironmentUtil. """

  def setUp(self) -> None:
    self.env_util = environment_variable_proxy.EnvironmentVariableProxy()

  @patch.dict('os.environ', {'ENVIRONMENT': 'development'})
  def test_should_properly_return_environment_variable_when_available(
      self) -> None:
    environment = self.env_util.get_environment_variable('ENVIRONMENT')

    self.assertEqual('development', environment)

  def test_should_properly_return_default_when_value_not_set(self) -> None:
    environment = self.env_util.get_environment_variable('ENVIRONMENT',
                                                         default='development')

    self.assertEqual('development', environment)
