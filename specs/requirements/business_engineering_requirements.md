...
|                |                                                                  |      |                                                               | **verbose** - Boolean valued query parameter indicating whether verbose response should be returned for each Spotify track ID. When true, the output for Spotify's `GET::/v1/tracks` bulk API is added to each recommended track ID. Default value false. |
